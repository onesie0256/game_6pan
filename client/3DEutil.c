...
void displayBillboards(void)
{
    ...
    if (scene->bulletTextureID != 0) {
        // Draw a test billboard at a fixed position
        Vec3f coord = vecAdd(scene->myCar->center , (Vec3f){0.0f,1.0f,0.0f});
        drawBillboard(coord, 1.0f, 1.0f, scene->bulletTextureID);
    }
}
